    ...
    return (
      <Layout>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          ...
          <Link to="/support-tickets" className="text-primary-600 hover:text-primary-700">
            ← Back to Tickets
          </Link>
        </div>
      </Layout>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(ticketStatus)}`}>
                {ticketStatus.replace('_', ' ').charAt(0).toUpperCase() + ticketStatus.replace('_', ' ').slice(1)}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg border border-wood-200 p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 text-sm">
            <div>
              <span className="text-charcoal-600">Customer:</span>
              <span className="ml-2 font-medium">{ticket.customerName}</span>
            </div>
            <div>
              <span className="text-charcoal-600">Email:</span>
              <span className="ml-2 font-medium">{ticket.customerEmail}</span>
            </div>
            <div>
            ...
            {ticket.orderId && (
              <div>
                ...
                <Link to={`/admin/orders`} className="ml-2 font-medium text-primary-600 hover:text-primary-700">
                  {ticket.orderId}
                </Link>
              </div>
            )}
